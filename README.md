# Java Projects Repository

This repository contains multiple Java projects, including:

## ToDoList Application
A simple To-Do List application built using Java Swing. It allows users to:
- Add tasks.
- Mark tasks as completed.
- Clear completed tasks.

### Features
- Dynamic task management.
- User-friendly interface.
- Customizable task states.

### File Structure
- `ToDoList/src/classes/`: Main Java classes for the ToDoList app.
- `ToDoList/.idea/`: IntelliJ IDEA project configuration files.

---

## Quiz Application
A Java-based Quiz Application that tests users' knowledge with multiple-choice questions.

### Features
- Timer for each question.
- 50-50 lifeline.
- Score tracking and display.

### File Structure
- `Quiz-Application-using-java/src/quiz/application/`: Main Java classes for the quiz app.
- `Quiz-Application-using-java/.idea/`: IntelliJ IDEA project configuration files.

---

## General Information
- Both projects are configured for IntelliJ IDEA.
- Java SDK version: 20.

### How to Run
1. Clone the repository.
2. Open the desired project in IntelliJ IDEA.
3. Run the main class of the respective application.

### License
This repository is licensed under the MIT License.
